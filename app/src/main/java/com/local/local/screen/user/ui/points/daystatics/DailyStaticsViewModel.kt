...
class DailyStaticsViewModel : ViewModel() {
    ...
    private val firebaseCallback = object : FirebaseCallback(){
        override fun retrieveRecord(recordInfo: RecordInfo?) {
            ...
            recordInfoImp.value = recordInfo ?: RecordInfo(days = calendarImp.value?.get(Calendar.DAY_OF_MONTH) ?: Calendar.getInstance(Locale.TAIWAN).get(Calendar.DAY_OF_MONTH))
        }
    }
