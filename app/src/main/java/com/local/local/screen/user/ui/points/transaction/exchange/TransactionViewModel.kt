...
class TransactionViewModel(private val eventManager: EventManager<Event> = EventManager()) :
    ViewModel(),
    EventBroadcaster<TransactionViewModel.Event> by eventManager {
    sealed class Event {
        class OnUpdateStart :
            Event()
    ...
    private val firebaseCallback = object : FirebaseCallback() {
        ...
        override fun retrieveStoreInfo(storeInfoList: List<StoreInfo>) {
            storeInfo.value?.clear()
            storeInfo.value?.addAll(storeInfoList)
